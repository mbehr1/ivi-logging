...
public:
    ThreadInformation() { m_id = sNextID++; }

    static bool isMultithreadedApp() { return (sNextID != 1); }
...
struct AppLogContext
{
    AppLogContext(const char* id, const char* description) : m_id(id), m_description(description)
    {
        if (s_pAppLogContext != nullptr) {
            fprintf(stderr,
                LOGGING_WARNING_OUTPUT_PREFIX "An ID has already been defined for your application. Please ensure "
                                              "LOG_DEFINE_APP_IDS macro is called before any log is produced.\n");
        }
...
public:
    LogContextCommon(const std::string& id, const std::string& contextDescription)
        : m_id(id), m_description(contextDescription)
    {
        if (id.length() > 4)
            fprintf(stderr,
...
public:
    bool isSourceCodeLocationInfoEnabled() const { return m_enableSourceCodeLocationInfo; }

    bool isThreadInfoEnabled() const { return m_enableThreadInfo; }

    bool isTimestampEnabled() const { return m_enableTimestamp; }
