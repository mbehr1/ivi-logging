...
static const std::chrono::time_point<std::chrono::steady_clock> m_start = std::chrono::steady_clock::now();
class StreamLogData;
...
public:
    ...
    void setParentContext(LogContextCommon& context) { m_context = &context; }

    const char* getShortID() { return m_context->getID(); }
    ...
    virtual bool isEnabled(LogLevel level) const { return (level <= getLogLevel()); }
...
public:
	 ...
	 */
    static void setGlobalLogLevel(LogLevel level) { s_defaultLogLevel = level; }

    FILE* getFile(StreamLogData& data) override;

    bool isColorsEnabled() { return m_colorSupport; }
...
public:
    ...
    static constexpr const char* DEFAULT_SUFFIX_WITH_SHORT_FILE_LOCATION_WITHOUT_FUNCTION_WITH_THREAD_NAME
        = " [ %s%.0s - %d | %.2i-%.16s ]";
    ...
    void setPrefixFormat(const char* format) { m_prefixFormat = format; }

    void setSuffixFormat(const char* format) { m_suffixFormat = format; }
    ...
    virtual void writeSuffix() { writeFormatted(getSuffix().getData()); }
    ...
    virtual ByteArray getSuffix()
    {
        ...
        if (m_context->isThreadInfoEnabled()) {
            writeFormatted(
                array, m_suffixThreadNameFormat, getThreadInformation().getID(), getThreadInformation().getName());
        }
    ...
    bool isEnabled() { return (m_context->isEnabled(m_data->getLogLevel())); }
...
template <size_t N> inline StreamLogData& operator<<(StreamLogData& data, const char (&v)[N])
{
    data << (const char*)v;
    return data;
}
...
public:
    ...
    virtual ~ConsoleLogData() { flushLog(); }
    ...
    void writeFooterColor() { writeHeaderColor(); }
