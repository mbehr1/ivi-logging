...
public:
    ...
    ~DltContextClass() { dlt_unregister_context(this); }
    ...
    void disable() { manuallyDisabled = true; }

    void enable() { manuallyDisabled = false; }
    ...
    virtual ~DltLogData()
    {
        if (m_enabled) {
            if (m_context->isSourceCodeLocationInfoEnabled()) {
                ...
                auto fileName = m_data->getFileName();
                if (fileName != nullptr)
                    dlt_user_log_write_utf8_string(this, fileName);
...
inline bool DltContextClass::isEnabled(LogLevel logLevel) const
{
    ...
    return ((this)->log_level_ptr && ((dltLogLevel) <= (int)*((this)->log_level_ptr))
        && ((dltLogLevel) != 0)); // TODO: could call dlt_user_is_logLevel_enabled
#endif
}
...
template <size_t N> inline DltLogData& operator<<(DltLogData& data, const char (&v)[N])
{
    data << (const char*)v;
    return data;
}
