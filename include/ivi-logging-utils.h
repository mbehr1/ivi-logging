...
public:
    ...
    size_t size() const { return (usesStaticBuffer() ? m_length : m_dynamicData->size()); }

    bool usesStaticBuffer() const { return (m_dynamicData == nullptr); }
    ...
    std::string toString() const { return byteArrayToString(getData(), size()); }
